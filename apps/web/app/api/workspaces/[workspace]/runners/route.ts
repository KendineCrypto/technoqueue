...
export async function DELETE(request: Request, context: Context) {
  try {
    ...
    run("UPDATE runner_jobs SET status = 'cancelled', result_text = CASE WHEN status = 'running' THEN 'Runner revoked while the job was in flight. Local changes may already exist.' ELSE result_text END, completed_at = ?, lease_expires_at = NULL, updated_at = ? WHERE runner_id = ? AND workspace_id = ? AND status IN ('awaiting_approval','queued','running')", timestamp, timestamp, input.runnerId, workspace.id);
