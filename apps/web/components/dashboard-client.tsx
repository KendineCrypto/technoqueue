...
export function DashboardClient({ username, did, initialWorkspaces }: { username: string; did: string; initialWorkspaces: Workspace[] }) {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-grid">
      ...
      <div className="workspace-list"><header><span>YOUR OFFICES</span><b>{workspaces.length}</b></header>{workspaces.length ? workspaces.map((workspace) => <Link href={`/board/${workspace.slug}`} className="workspace-row" key={workspace.id}><div><strong>{workspace.name}</strong><code>{workspace.slug}</code></div><ArrowRight size={18}/></Link>) : <div className="dashboard-empty">No office yet. Open your first floor and become the boss.</div>}</div>
    </section>
