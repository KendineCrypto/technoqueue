...
function RunnerDialog({ workspace, runners, projects, jobs, onClose, onChanged }: { workspace: string; runners: LocalRunner[]; projects: LocalProject[]; jobs: LocalJob[]; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  async function updateJob(job: LocalJob, action: "approve" | "reject" | "retry") {
    if (action === "approve" && job.highRisk && !window.confirm(job.kind === "verify"
      ? "This command will execute project code on your computer. It may read local files or use your network and credentials. Approve only if you trust this project and command."
      : "This proposal changes a dependency, lockfile, package manifest, or CI configuration. Review every byte before approving.")) return;
    setBusy(true); setError("");
    try {
      const payload = action === "approve" ? { jobId: job.id, action, requestSha256: job.requestSha256, acknowledgeRisk: job.highRisk } : { jobId: job.id, action };
      const response = await fetch(`/api/workspaces/${encodeURIComponent(workspace)}/jobs`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      const responseBody = await response.json() as { error?: string }; if (!response.ok) throw new Error(responseBody.error ?? "Local job could not be updated"); await onChanged();
