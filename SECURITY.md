...
The optional v0.3.3 local runner is a separate, explicit trust boundary. Local absolute paths stay in the runner config. A project needs an owner-approved grant, and every file write or verification command needs a separate owner approval bound to the exact request hash. The runner validates relative paths, rejects symlink/protected/generated paths, uses fixed pinned command entrypoints without a shell, caps output and time, and signs each receipt. Runner jobs expire and permissions are checked again when their receipt arrives. Filtered source snapshots are encrypted at rest, hidden from browser APIs, and purged after the change proposal is created or after a configurable 24-hour default TTL. This is not a complete sandbox: an approved package script is project-controlled code and can do anything the operating-system user can do. Run the bridge under a low-privilege account with minimal environment credentials, keep projects in version control, inspect every proposal, and never approve an unfamiliar command.
